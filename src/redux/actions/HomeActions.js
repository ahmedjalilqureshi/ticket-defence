...
export const setImage2 = (state) => (dispatch) => {
  dispatch({ type: "SET_IMAGE2", payload: state });
};
export const setImage3 = (state) => (dispatch) => {
  dispatch({ type: "SET_IMAGE3", payload: state });
};
export const setImage4 = (state) => (dispatch) => {
  dispatch({ type: "SET_IMAGE4", payload: state });
};
export const setImage5 = (state) => (dispatch) => {
  dispatch({ type: "SET_IMAGE5", payload: state });
};
export const setImage6 = (state) => (dispatch) => {
  dispatch({ type: "SET_IMAGE5", payload: state });
};
...
export const setLicensePoints2 = (points) => (dispatch) => {
  dispatch({ type: "SET_LICENSE_POINTS2", payload: points });
};
...
export const createToken = (data) => (dispatch) =>
  new Promise((resolve, reject) => {
    const { crediCardNumber, cvv,name, date, state } = data;
    NetInfo.fetch().then((net) => {
      if (net.isConnected) {
        axios
          .post(
  ...
  new Promise((resolve, reject) => {
    NetInfo.fetch().then((net) => {
      if (net.isConnected) {
        ...
        axios
          .post(
            `https://api.stripe.com/v1/charges?source=${token}&key=${stripeSecretKey}&amount=${amount * 100
            }&currency=usd`
          )
          .then((res) => {
            resolve(res.data);
